...
void sys_GPS()
{
   u8 tp_last, key_value, i, is_key = 0;
		...
		while(1)
		{
		LCD_printf(0,6+36*0,300,24,24,"========GPS=========");
		LCD_printf(0,6+36*1,300,24,24,"Angle:%lf",GPS.radian*180/pi);
		LCD_printf(0,6+36*2,300,24,24,"X:%lf",GPS.position.x);
		LCD_printf(0,6+36*3,300,24,24,"Y:%lf",GPS.position.y);
		LCD_printf(0,6+36*4,300,24,24,"1.SetA 2.SetX 3.SetY");
		LCD_printf(0,6+36*5,300,24,24,"4.Clear");
		LCD_printf(0,6+36*6,300,24,24,"5.Record 6.Set");
			if (Is_USART_Key_Receive)
		{
				Is_USART_Key_Receive = 0;
				key_value = Key_RxBuffer[0];
